...
My favorite idea is to use an adaptor point corresponding to an oracle’s signature of a real-world event, so that until a signature is broadcasted the adaptor signature cannot be decrypted which enables us to create (using only signatures) Bitcoin payments and contracts contingent on real-world events, these are called [Discreet Log Contracts or DLCs](https://web.archive.org/web/20241113012850/https://suredbits.com/discreet-log-contracts-part-1-what-is-a-discreet-log-contract/).
